...
public class Compute 
{
	...
	public int getNextPosition(String str, int start)
	{
		...
		return result;
	}//Написать функцию исппользующую внешний счётчик для определения следующего символа. Предпологаемый возвращаемый тип int.
	...
	public boolean isCorrect(String str)
	{
		...
		for(int i = 0; i < str.length(); i++)
		{
			...
			case NUM:
				...
				if(next_value == NUM)//Пробел не должен заносится в значения value
				{
					System.out.println("Missing operator between two operands on position " + (i+1));
				}
			//Добавить выбрасываемое значение OPERATOR (+,-,*,/,^)
